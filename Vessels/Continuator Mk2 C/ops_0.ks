...
UNTIL opsDone	{
	...
	IF opState = 3 AND timer >= timeout AND ship_pitch < 50{
	...
	IF opState = 7 AND ETA:APOAPSIS < 120{
	...
	IF opState = 8 AND timer >= timeout AND ship:verticalspeed > 0{
		...
		LOCK THROTTLE TO ((20-ETA:APOAPSIS)/20).
	...
	IF opState = 9 AND timer > timeout AND (ABS(ship:apoapsis - ship:periapsis) < 1500) AND ship:periapsis > 70000{
